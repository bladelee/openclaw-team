...
COMMENT ON COLUMN instances.custom_url IS 'Custom URL for external instances (custom or hardware)';
COMMENT ON COLUMN instances.health_check_url IS 'Custom health check endpoint for external instances';
COMMENT ON COLUMN instances.health_check_interval IS 'Health check interval in seconds';
COMMENT ON COLUMN instances.last_health_check IS 'Timestamp of last health check';
COMMENT ON COLUMN instances.is_healthy IS 'Health status from last health check (NULL for managed instances)';
