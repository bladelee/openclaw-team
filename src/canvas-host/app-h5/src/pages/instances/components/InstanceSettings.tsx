...
export function InstanceSettings({ instance, onClose, onRefresh }: InstanceSettingsProps) {
  const [name, setName] = useState(instance.name);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const styles = useThemeStyles(); // 使用主题样式
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try {
        ...
        if (config.userRole) setUserRole(config.userRole);
      } catch (e) {
  ...
  const getStatusText = (): string => {
    ...
    return textMap[instance.status] || "未知";
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Gateway 连接配置 */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">Gateway 连接</h3>
        <div
          className="rounded-xl p-4 space-y-4"
          style={{
            backgroundColor: "var(--color-surface)",
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              style={{
                backgroundColor: isConnected ? "var(--color-border)" : "#0f1720",
                border: "1px solid var(--color-border)",
                color: "var(--color-text)",
                outline: "none",
              }}
          ...
          {/* 认证 Token */}
          <div className="space-y-2">
            <label className="text-sm text-[var(--color-text-tertiary)]">认证 Token（可选）</label>
            <input
              type="password"
              value={authToken}
